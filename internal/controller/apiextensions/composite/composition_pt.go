...
func NewAPIDryRunRenderer(c client.Client) *APIDryRunRenderer {
	return &APIDryRunRenderer{client: c, internalRenderer: NewPureRenderer()}
}
...
func (r *APIDryRunRenderer) Render(ctx context.Context, cp resource.Composite, cd resource.Composed, t v1.ComposedTemplate, env *env.Environment) error {
	if err := r.internalRenderer.Render(ctx, cp, cd, t, env); err != nil {
		return err
	}
...
// A PureRenderer renders composed resources. It does not perform any API call.
type PureRenderer struct{}

// NewPureRenderer returns a PureRenderer of composed resources.
func NewPureRenderer() *PureRenderer {
	return &PureRenderer{}
}

// Render the supplied composed resource using the supplied composite resource
// and template.
func (r *PureRenderer) Render(_ context.Context, cp resource.Composite, cd resource.Composed, t v1.ComposedTemplate, env *env.Environment) error { //nolint:gocyclo // Only slightly over (11).

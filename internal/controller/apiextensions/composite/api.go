...
const (
	...
	errParseCompositionFromManifest    = "cannot parse composition from revision manifest"
)
...
func (f *APIRevisionFetcher) Fetch(ctx context.Context, cr resource.Composite) (*v1.Composition, error) {
	...
	if ref != nil && pol != nil && *pol == xpv1.UpdateManual {
		rev := &v1alpha2.CompositionRevision{}
		if err := f.ca.Get(ctx, meta.NamespacedNameOf(ref), rev); err != nil {
			return nil, errors.Wrap(err, errGetCompositionRevision)
		}
		return AsComposition(rev)
	}
	...
	if ref == nil || ref.Name != current.GetName() {
		cr.SetCompositionRevisionReference(meta.ReferenceTo(current, v1alpha2.CompositionRevisionGroupVersionKind))
		if err := f.ca.Apply(ctx, cr); err != nil {
			return nil, errors.Wrap(err, errUpdate)
		}
	}

	return AsComposition(current)
}

// AsComposition creates a new composition from the supplied revision.
// A compositio revision stores a JSON manifest for a composition type.
func AsComposition(cr *v1alpha2.CompositionRevision) (*v1.Composition, error) {
	comp := &v1.Composition{}
	err := json.Unmarshal(cr.Spec.Composition.Raw, comp)
	return comp, errors.Wrap(err, errParseCompositionFromManifest)
}

func (f *APIRevisionFetcher) getCompositionRevisionList(ctx context.Context, cr resource.Composite, comp *v1.Composition) (*v1alpha2.CompositionRevisionList, error) {
	...
	ml[v1alpha2.LabelCompositionName] = comp.GetName()

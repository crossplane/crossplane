	...
	for _, fn := range req.Revision.Spec.Pipeline {
		...
		for _, cs := range fn.Credentials {
			...
			if err := c.client.Get(ctx, client.ObjectKey{Namespace: cs.SecretRef.Namespace, Name: cs.SecretRef.Name}, s); err != nil {
				...
				if kerrors.IsNotFound(err) && isOptionalCredential(cs) {
...
// isOptionalCredential returns true if the credential is marked as optional.
// When a credential is optional and its secret is not found, the pipeline
// continues without the credential rather than failing.
func isOptionalCredential(fc v1.FunctionCredentials) bool {
	return fc.Optional != nil && *fc.Optional
}

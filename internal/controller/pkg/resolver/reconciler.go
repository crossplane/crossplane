...
const (
	...
	errFindDependencyUpdate = "cannot find dependency version to update"
...
// WithUpdatesEnabled sets whether updates are enabled or not.
func WithUpdatesEnabled() ReconcilerOption {
	return func(r *Reconciler) {
		r.updatesEnabled = true
	}
}
...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	...
	if o.Features.Enabled(features.EnableAlphaDependencyVersionUpdate) {
		opts = append(opts, WithUpdatesEnabled(), WithNewDagFn(internaldag.NewUpdatableMapDag))
	}
...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) { //nolint:gocognit // we need to handle both pkg installation and upgrade scenarios now
	...
	if pkg == nil {
		...
		if addVer == "" {
			log.Debug(errFindDependencyUpdate, "error", errors.Errorf(errFmtNoValidVersion, depID, dep.Constraints))
	...
	// The package is installed, but does not satisfy the constraints and updates are enabled.
	...
	newVer, err := r.findDependencyVersionToUpdate(ctx, ref, insVer, n, log)
	if err != nil {
		log.Debug(errFindDependencyUpdate, "error", errors.Wrapf(err, depID, dep.Constraints))
		r.record.Event(lock, event.Warning(reasonNoValidVersion, err))
		return reconcile.Result{}, errors.Wrap(err, errFindDependencyUpdate)
	}
...
func (r *Reconciler) findDependencyVersionToUpdate(ctx context.Context, ref name.Reference, insVer string, dep internaldag.Node, log logging.Logger) (string, error) {
	...
	log.Debug(errFindDependencyUpdate, "error", errors.Errorf(errFmtNoValidVersion, dep.Identifier(), dep.GetParentConstraints()))
...
func splitPackage(p string) (repo, version string, err error) {
	...
	if len(a) != 2 {
		return "", "", errors.Errorf("package should have 2 segments after split but has %d", len(a))
	}
	return a[0], a[1], nil
}

// findDigestToUpdate returns the digest to update if all parent constraints are the same digest.
// It returns an error, if there is at least one digest which is different than other constraints.
	...
	for _, c := range node.GetParentConstraints() {
		if d, err := conregv1.NewHash(c); err == nil {
			if foundDigest != "" && foundDigest != d.String() {
				return "", errors.Errorf("a package with digest has different parent constraints (%v)", node.GetParentConstraints())
			}

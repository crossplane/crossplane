...
func TestMultiRevisionActivator(t *testing.T) {
	errBoom := errors.New("boom")

	type args struct {
		act  *MultiRevisionActivator
		pkg  v1.Package
		revs []v1.PackageRevision
	}
	...
	cases := map[string]struct {
		...
		"AutomaticMultiActivationOneRevision": {
			reason: "The activator should activate the current revision when it's not already active in multi-revision mode with only one revision.",
			args: args{
				act: &MultiRevisionActivator{
					client: resource.ClientApplicator{
						Applicator: resource.ApplyFn(func(_ context.Context, o client.Object, _ ...resource.ApplyOption) error {
				...
				pkg: &v1.Configuration{
					Spec: v1.ConfigurationSpec{
						PackageSpec: v1.PackageSpec{
							...
							ActiveRevisionLimit:      ptr.To(int64(2)),
						},
					},
					Status: v1.ConfigurationStatus{
						PackageStatus: v1.PackageStatus{
		...
		"AutomaticMultiActivationTwoRevisions": {
			reason: "The activator should activate two revisions in multi-revision mode with two revisions installed.",
			args: args{
				act: &MultiRevisionActivator{
					client: resource.ClientApplicator{
						Applicator: resource.ApplyFn(func(_ context.Context, o client.Object, _ ...resource.ApplyOption) error {
							var wantState v1.PackageRevisionDesiredState
							switch o.GetName() {
							case "revision-1":
								wantState = v1.PackageRevisionRuntimeOnly
							case "revision-2":
								wantState = v1.PackageRevisionActive
							default:
								return errors.New("unexpected revision name")
							}
							want := &v1.ConfigurationRevision{
								ObjectMeta: metav1.ObjectMeta{
									Name: o.GetName(),
			...
			want: want{
				...
				revs: []v1.PackageRevision{
					...
					&v1.ConfigurationRevision{
						...
						Spec: v1.PackageRevisionSpec{
							Revision:     1,
							DesiredState: v1.PackageRevisionRuntimeOnly,
						},
					},
				},
			},
		},
		"AutomaticMultiActivationThreeRevisions": {
			reason: "The activator should activate two revisions in multi-revision mode with three revisions installed.",
			args: args{
				act: &MultiRevisionActivator{
					client: resource.ClientApplicator{
						Applicator: resource.ApplyFn(func(_ context.Context, o client.Object, _ ...resource.ApplyOption) error {

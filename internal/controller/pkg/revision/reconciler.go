...
func SetupProviderRevision(mgr ctrl.Manager, o controller.Options) error {
	...
	ro := []ReconcilerOption{
		WithCache(o.Cache),
		WithDependencyManager(NewPackageDependencyManager(mgr.GetClient(), dag.NewUpdatableMapDag, v1beta1.ProviderPackageType)),
...
func SetupConfigurationRevision(mgr ctrl.Manager, o controller.Options) error {
	...
	r := NewReconciler(mgr,
		WithCache(o.Cache),
		WithDependencyManager(NewPackageDependencyManager(mgr.GetClient(), dag.NewUpdatableMapDag, v1beta1.ConfigurationPackageType)),
		WithNewPackageRevisionFn(nr),
		WithEstablisher(NewAPIEstablisher(mgr.GetClient(), o.Namespace, o.MaxConcurrentPackageEstablishers)),
		WithParser(parser.New(metaScheme, objScheme)),
...
func SetupFunctionRevision(mgr ctrl.Manager, o controller.Options) error {
	...
	ro := []ReconcilerOption{
		WithCache(o.Cache),
		WithDependencyManager(NewPackageDependencyManager(mgr.GetClient(), dag.NewUpdatableMapDag, v1beta1.FunctionPackageType)),
		WithEstablisher(NewAPIEstablisher(mgr.GetClient(), o.Namespace, o.MaxConcurrentPackageEstablishers)),
		WithNewPackageRevisionFn(nr),
		WithParser(parser.New(metaScheme, objScheme)),
		WithParserBackend(NewImageBackend(fetcher, WithDefaultRegistry(o.DefaultRegistry))),

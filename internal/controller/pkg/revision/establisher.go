...
func (e *APIEstablisher) validate(ctx context.Context, objs []runtime.Object, parent v1.PackageRevision, control bool) (allObjs []desiredApply, err error) { //nolint:gocognit // TODO(negz): Refactor this to break up complexity.
	...
	for _, res := range objs {
		g.Go(func() error {
			...
			if kerrors.IsNotFound(err) {
				if control {
					apply, _, err := e.prepareApply(ctx, nil, desired, parent, true)
					if err != nil {
						return err
					}
			...
			apply, needed, err := e.prepareApply(ctx, currentResource, desired, parent, control)
			if err != nil {
				return err
			}
			if needed {
...
func (e *APIEstablisher) prepareApply(ctx context.Context, current, desired, parent resource.Object, control bool) (*unstructured.Unstructured, bool, error) {
	...
	if control {
		...
		// A revision we're replacing may still be the object's controller,
		// because it hasn't relinquished control yet or because it was
		// replaced or deleted before it could. Demote it to be just an owner,
		// so the hand-off doesn't depend on the order in which the two
		// revisions reconcile.
		if err := e.demoteOldController(ctx, apply, parent, ctrlRef); err != nil {
			return nil, false, err
		}

		if err := meta.AddControllerReference(apply, ctrlRef); err != nil {
			return nil, false, err
		}

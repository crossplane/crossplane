...
func TestAPIEstablisherPrepareApply(t *testing.T) {
	...
	t.Run("CreateWithControllerAndPackageOwners", func(t *testing.T) {
		...
		apply, needed, err := newAPIEstablisher(nil).prepareApply(context.Background(), nil, desired, parent, true)
		if err != nil {
			t.Fatal(err)
		}
		if !needed {
	...
	t.Run("DetectControlledIntentChange", func(t *testing.T) {
		...
		_, needed, err := newAPIEstablisher(nil).prepareApply(context.Background(), current, desired, parent, true)
		if err != nil {
			t.Fatal(err)
		}
		if !needed {
	...
	t.Run("PreserveExistingOwnersWithoutControl", func(t *testing.T) {
		...
		apply, needed, err := newAPIEstablisher(nil).prepareApply(context.Background(), current, desired, parent, false)
		if err != nil {
			t.Fatal(err)
		}
		if !needed {
	...
	t.Run("OmitExistingMRDState", func(t *testing.T) {
		...
		apply, _, err := newAPIEstablisher(nil).prepareApply(context.Background(), current, desired, parent, true)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("RejectForeignController", func(t *testing.T) {
		...
		if _, _, err := newAPIEstablisher(nil).prepareApply(context.Background(), current, desired, parent, true); err == nil {
			t.Error("prepareApply(...) accepted an object controlled by a foreign owner")
		}
	})

	t.Run("TakeControlFromReplacedRevision", func(t *testing.T) {
		oldRevisionRef := metav1.OwnerReference{
			APIVersion: v1.SchemeGroupVersion.String(),
			Kind:       v1.ProviderRevisionKind,
			Name:       "old-revision-name",
			UID:        "old-revision-uid",
			Controller: ptr.To(true),
		}
		current := &corev1.ConfigMap{
			TypeMeta:   metav1.TypeMeta{APIVersion: corev1.SchemeGroupVersion.String(), Kind: "ConfigMap"},
			ObjectMeta: metav1.ObjectMeta{Name: "test", OwnerReferences: []metav1.OwnerReference{pkgRef, oldRevisionRef}},
		}
		desired := current.DeepCopy()
		desired.SetOwnerReferences(nil)

		// The old revision is inactive - it's being replaced by parent, but
		// hasn't relinquished control of this object yet (or never will,
		// because it was paused, deleted, or replaced).
		oldRevision := &v1.ProviderRevision{
			ObjectMeta: metav1.ObjectMeta{
				Name: "old-revision-name",
				UID:  "old-revision-uid",
				Labels: map[string]string{
		...
		oldRevision.SetDesiredState(v1.PackageRevisionInactive)

		e := newAPIEstablisher(&test.MockClient{
			MockGet: func(_ context.Context, _ client.ObjectKey, obj client.Object) error {
				oldRevision.DeepCopyInto(obj.(*v1.ProviderRevision))
				return nil
			},
		})

		apply, needed, err := e.prepareApply(context.Background(), current, desired, parent, true)
		if err != nil {
			t.Fatal(err)
		}
		if !needed {
			t.Error("prepareApply(...) did not report a change from taking control")
		}

		demoted := oldRevisionRef
		demoted.Controller = ptr.To(false)
		want := []metav1.OwnerReference{pkgRef, demoted, controllerRef}
		if diff := cmp.Diff(want, apply.GetOwnerReferences()); diff != "" {
			t.Errorf("prepareApply(...): -want owners, +got owners:\n%s", diff)
		}
	})

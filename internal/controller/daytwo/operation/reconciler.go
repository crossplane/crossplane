...
import (
	...
	fnv1alpha1 "github.com/crossplane/crossplane/apis/daytwo/fn/proto/v1alpha1"
...
type FunctionRunner interface {
	...
	RunFunction(ctx context.Context, name string, req *fnv1alpha1.RunFunctionRequest) (*fnv1alpha1.RunFunctionResponse, error)
}

// A FunctionRunnerFn is a function that can run an operation function.
type FunctionRunnerFn func(ctx context.Context, name string, req *fnv1alpha1.RunFunctionRequest) (*fnv1alpha1.RunFunctionResponse, error)

// RunFunction runs the named Composition Function with the supplied request.
func (fn FunctionRunnerFn) RunFunction(ctx context.Context, name string, req *fnv1alpha1.RunFunctionRequest) (*fnv1alpha1.RunFunctionResponse, error) {
...
type ExtraResourcesFetcher interface {
	Fetch(ctx context.Context, rs *fnv1alpha1.ResourceSelector) (*fnv1alpha1.Resources, error)
}

// An ExtraResourcesFetcherFn gets extra resources matching the selector.
type ExtraResourcesFetcherFn func(ctx context.Context, rs *fnv1alpha1.ResourceSelector) (*fnv1alpha1.Resources, error)

// Fetch gets extra resources matching the selector.
func (fn ExtraResourcesFetcherFn) Fetch(ctx context.Context, rs *fnv1alpha1.ResourceSelector) (*fnv1alpha1.Resources, error) {
	return fn(ctx, rs)
}
...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) { //nolint:gocognit // Reconcilers are typically complex.
	...
	for _, fn := range op.Spec.Pipeline {
		...
		req := &fnv1alpha1.RunFunctionRequest{Desired: d, Context: fctx}
		...
		var reqs *fnv1alpha1.Requirements

		// Used to store the response of the function at the previous iteration.
		var rsp *fnv1alpha1.RunFunctionResponse
		...
		for i := int64(0); i <= MaxRequirementsIterations; i++ {
			...
			req.ExtraResources = make(map[string]*fnv1alpha1.Resources)
...
func (r *PooledFunctionRunner) RunFunction(ctx context.Context, name string, req *fnv1alpha1.RunFunctionRequest) (*fnv1alpha1.RunFunctionResponse, error) {
	conn, err := r.pool.GetConnection(ctx, name)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot get gRPC client connection for Function %q", name)
	}
	...
	rsp, err := fnv1alpha1.NewFunctionRunnerServiceClient(conn).RunFunction(ctx, req)
	return rsp, errors.Wrapf(err, "cannot run Function %q", name)
}
...
func (e *ExistingExtraResourcesFetcher) Fetch(ctx context.Context, rs *fnv1alpha1.ResourceSelector) (*fnv1alpha1.Resources, error) {
	if rs == nil {
		return nil, errors.New("resource selector must not be nil")
	}

	switch match := rs.GetMatch().(type) {
	case *fnv1alpha1.ResourceSelector_MatchName:
		...
		return &fnv1alpha1.Resources{Items: []*fnv1alpha1.Resource{{Resource: o}}}, nil

	case *fnv1alpha1.ResourceSelector_MatchLabels:
		...
		rs := make([]*fnv1alpha1.Resource, len(list.Items))
		for i := range list.Items {
			o, err := AsStruct(&list.Items[i])
			if err != nil {
				return nil, errors.Wrap(err, "cannot convert extra resource to protobuf Struct")
			}
			rs[i] = &fnv1alpha1.Resource{Resource: o}
		}

		return &fnv1alpha1.Resources{Items: rs}, nil
	}

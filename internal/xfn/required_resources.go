...
func (c *FetchingFunctionRunner) RunFunction(ctx context.Context, name string, req *fnv1.RunFunctionRequest) (*fnv1.RunFunctionResponse, error) { //nolint:gocognit // It's essentially one loop with a series of flat conditionals; it doesn't read as complex.
	...
	for i := int32(0); i <= MaxRequirementsIterations; i++ {
		...
		// On the function's first call, if the request we already sent it
		// satisfied its requirements, it saw everything it needs - its response
		// is final, so we don't call it again. This is what lets a request
		// seeded with a function's requirements (a Composition's bootstrap
		// requirements, or resources it required last reconcile) resolve in a
		// single call rather than two: one to learn the requirements, another to
		// use them. Later iterations fall through to the requirements-stabilized
		// check above, so a function whose requirements never settle still
		// errors rather than short-circuiting here.
		if i == 0 &&

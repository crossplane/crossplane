...
func TestCompositionValidation(t *testing.T) {
	cases := map[string]struct {
		reason       string
		current, old *v1.Composition
		validatorFn  func(obj, old *v1.Composition) field.ErrorList
		wantErrs     []string
	}{
		"CreateEmpty": {
...
func TestCompositionRevisionValidation(t *testing.T) {
	cases := map[string]struct {
		reason       string
		current, old *v1.CompositionRevision
		validatorFn  func(obj, old *v1.CompositionRevision) field.ErrorList
		wantErrs     []string
	}{
		"CreateEmpty": {
			reason:      "Attempted to create an empty CompositionRevision.",
			current:     resources.New[v1.CompositionRevision](t),
			validatorFn: resources.ValidatorFor[v1.CompositionRevision](t),
			wantErrs:    []string{},
		},
	}

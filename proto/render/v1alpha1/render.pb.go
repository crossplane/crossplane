...
type CompositeInput struct {
	...
	// CompositeResourceDefinitions are the XRDs the binary should consider when
	// rendering. The binary looks up the XRD whose composite GVK matches the
	// input XR to determine the right composite.Schema (Legacy vs Modern),
	// mirroring what the production reconciler does.
	//
	// Optional. If absent, the binary falls back to its current default
	// (Schema=Modern), preserving back-compat for existing callers.
	CompositeResourceDefinitions []*structpb.Struct `protobuf:"bytes,8,rep,name=composite_resource_definitions,json=compositeResourceDefinitions,proto3" json:"composite_resource_definitions,omitempty"`
	unknownFields                protoimpl.UnknownFields
	sizeCache                    protoimpl.SizeCache
}
...
func (x *CompositeInput) GetCompositeResourceDefinitions() []*structpb.Struct {
	if x != nil {
		return x.CompositeResourceDefinitions
	}
	return nil
}
...
const file_proto_render_v1alpha1_render_proto_rawDesc = "" +
	...
	"\amessage\x18\x03 \x01(\tR\amessage\"\xca\x04\n" +
	...
	"\x1ecomposite_resource_definitions\x18\b \x03(\v2\x17.google.protobuf.StructR\x1ccompositeResourceDefinitions\"\xae\x03\n" +
...
var file_proto_render_v1alpha1_render_proto_depIdxs = []int32{
	...
	14, // 17: crossplane.render.v1alpha1.CompositeInput.composite_resource_definitions:type_name -> google.protobuf.Struct

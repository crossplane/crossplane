...
// Package operation implements the 'crossplane internal render operation'
// subcommand.
package operation
...
// Command renders an operation using the real Operation reconciler.
type Command struct {
	Timeout time.Duration `default:"2m" help:"Timeout for the render operation."`
}

// Run reads operation.Input from stdin and writes operation.Output to stdout.
func (c *Command) Run(log logging.Logger) error {
	...
	out, err := operation.Render(ctx, log, in)
	if err != nil {
		return errors.Wrap(err, "cannot render operation")
	}

	out.APIVersion = operation.APIVersion
	out.Kind = operation.KindOutput

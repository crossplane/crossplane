...
#
set -e -o pipefail
ROLES=(roles/iam.serviceAccountUser roles/cloudsql.admin roles/container.admin)
SERVICES=(container.googleapis.com sqladmin.googleapis.com)
...
read -e -i "$ORGANIZATION_ID" -p "Choose an Organization ID: " ORGANIZATION_ID
...
if [[ -z $EXAMPLE_PROJECT_ID_FOUND ]]; then
	...
	read -e -i "$ACCOUNT_ID" -p "Choose a Billing Account ID: " ACCOUNT_ID

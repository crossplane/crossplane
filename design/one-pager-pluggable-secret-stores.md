...
For deploying External Secret Store plugins, we will introduce a new package type
`Secret Store`. This package type will be similar to the existing `Provider`
type. It will contain a CRD to configure the secret store plugin (e.g., `VaultConfig`), 
a k8s service and a deployment. Required RBAC resources will be generated by the
package manager.

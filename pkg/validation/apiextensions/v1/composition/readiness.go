...
func (v *Validator) validateReadinessChecksWithSchemas(
	ctx context.Context,
	comp *v1.Composition,
	...
	for i, resource := range comp.Spec.Resources {
		...
		for j, r := range resource.ReadinessChecks {
			// TODO(phisco): we should validate also other fields of the ReadinessCheck
			if r.FieldPath == "" {
				continue
			}
			...
			matchType := getReadinessCheckExpectedType(r)
			if matchType == "" {
				continue
			}
			if matchType != fieldType {

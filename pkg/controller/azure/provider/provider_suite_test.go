...
func (tm *TestManager) createProvider(p *azurev1alpha1.Provider) error {
	return tm.reconciler.Create(ctx, p)
}

func (tm *TestManager) getProvider() (*azurev1alpha1.Provider, error) {
	p := &azurev1alpha1.Provider{}
	return p, tm.reconciler.Get(ctx, expectedRequest.NamespacedName, p)
}

func (tm *TestManager) deleteProvider(p *azurev1alpha1.Provider) error {
	return tm.reconciler.Delete(ctx, p)
}

func testSecret(secretName, secretDataKey string, data []byte) *corev1.Secret {
...
type MockValidator struct {
	MockValidate func(*azureclient.Client) error
}

// Validate - default implementation is always valid
func (mv *MockValidator) Validate(client *azureclient.Client) error {
	if mv.MockValidate != nil {
		return mv.MockValidate(client)
	}
	return nil
}

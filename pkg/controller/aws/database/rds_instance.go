...
// AddCloudsqlInstance creates a new RDSInstance Controller and adds it to the Manager with default RBAC. The Manager will set fields on the Controller
// and Start it when the Manager is Started.
// USER ACTION REQUIRED: update cmd/manager/main.go to call this rds.AddCloudsqlInstance(mgr) to install this Controller
func AddCloudsqlInstance(mgr manager.Manager) error {
	return addRDSInstanceReconciler(mgr, newRDSInstanceReconciler(mgr))
}

// newRDSInstanceReconciler returns a new reconcile.Reconciler
func newRDSInstanceReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileRDSInstance{Client: mgr.GetClient(), scheme: mgr.GetScheme()}
}
...
func addRDSInstanceReconciler(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	// Uncomment watch a Deployment created by RDSInstance - change this for objects you create
...
// +kubebuilder:rbac:groups=rds.aws.conductor.io,resources=instances,verbs=get;list;watch;create;update;patch;delete

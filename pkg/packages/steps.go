...
// installStep unmarshals install.yaml bytes to API machinery Unstructured which is set on the PackagePackager
func installStep(sp PackagePackager) walker.Step {
...
// iconStep unmarshals icon.* bytes to IconSpec which is added to the PackagePackager
func iconStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
...
// appStep unmarshals app.yaml bytes to AppMetadataSpec which is set on the PackagePackager
func appStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
...
func behaviorStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
...
// crdStep unmarshals crd.yaml bytes to a Kubernetes CRD which is added to the PackagePackager
func crdStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
		...
		packageIsNamespacedScope := sp.IsNamespaced()
		crdIsNotNamespacedScope := (crd.Spec.Scope != apiextensions.NamespaceScoped) && (crd.Spec.Scope != "")

		if packageIsNamespacedScope && crdIsNotNamespacedScope {
			return errors.New(fmt.Sprintf("Package CRD %q must be namespaced scope", path))
		}
...
// groupStep unmarshals group.yaml bytes to a PackageGroup which is added to the PackagePackager
func groupStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
		sg := &PackageGroup{}
...
// resourceStep unmarshals resource.yaml bytes to a PackageResource which is added to the PackagePackager
func resourceStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {
		sr := &PackageResource{}
...
// uiStep adds ui-schema.yaml bytes to the PackagePackager
func uiStep(sp PackagePackager) walker.Step {
	return func(path string, b []byte) error {

...
go-lint:
  ...
  CACHE --id go-build --sharing shared /root/.cache/go-build
  # Install with the release's own script rather than the one on master, so
  # the script and the assets it verifies always come from the same release.
